...
def run(label,gv,rv,tcache,threshold):
 ...
 config=json.loads(db.execute('select document_json from config_documents').fetchone()[0]);config['host_mappings']=[{'host':'business.local','target':'https://127.0.0.1:28081','target_type':'proxy','use_auth':False,'suppress_toolbar':True}];config['host_mappings'] += [{'host':f'unused-{i:04d}.local','target':'https://127.0.0.1:28081','target_type':'proxy','use_auth':False,'suppress_toolbar':True} for i in range(512)];config['run_type']=3;config['auto_manage_firewall']=False
 ...
 try:
  ...
  time.sleep(1)
cells=[('business_old_old',2412,2412,7,131072),('business_new_new',2415,2415,0,32768),('business_old_new',2412,2415,0,32768),('business_new_old',2415,2412,7,131072)]
