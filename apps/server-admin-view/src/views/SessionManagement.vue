...
const showSessionsTab = computed(() => configStore.config?.run_type === 1)
const defaultTab = computed(() => (showSessionsTab.value ? 'sessions' : 'login-backoff'))
...
<template>
  <div class="h-full flex flex-col gap-4">
    <Tabs :model-value="currentTab" @update:model-value="navigateTo" class="w-full">
      <TabsList>
        <TabsTrigger v-if="showSessionsTab" value="sessions">会话管理</TabsTrigger>
