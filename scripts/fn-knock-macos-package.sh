...
GO_COMMIT="${FN_KNOCK_GO_SOURCE_COMMIT:-}"
...
GO_COMMIT="${GO_COMMIT:-${ACTUAL_GO_COMMIT}}"
printf '%s\n' "${GO_COMMIT}" | grep -Eq '^[0-9a-fA-F]{40}$' || fail "invalid Go source commit"
[ "$(printf '%s' "${GO_COMMIT}" | tr '[:upper:]' '[:lower:]')" = "$(printf '%s' "${ACTUAL_GO_COMMIT}" | tr '[:upper:]' '[:lower:]')" ] || \
  fail "Go checkout ${ACTUAL_GO_COMMIT} does not match ${GO_COMMIT}"
