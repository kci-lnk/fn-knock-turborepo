  ...
  <Card data-guide-auth-settings class="min-h-[600px]">
    <CardHeader class="flex flex-row items-center justify-between">
      <div>
        ...
        <CardDescription>管理管理员登录使用的所有 TOTP 双端验证器。</CardDescription>
      </div>
        ...
        <Table>
          ...
          <TableBody>
            <TableRow v-for="n in 4" :key="n">
              ...
              <TableCell class="text-right"><Skeleton class="h-8 w-24 rounded-md ml-auto" /></TableCell>
            </TableRow>
          </TableBody>
        </Table>
      </div>
        ...
        <TableBody>
          <TableRow v-for="totp in credentials" :key="totp.id">
            ...
            <TableCell>
              ...
              <Button variant="link" class="p-0 h-auto" @click="goToPasskeys(totp.id)">
                管理 Passkeys
              </Button>
            </TableCell>
    ...
    <DialogContent class="max-w-md !top-[5vh] !translate-y-0 max-h-[85vh] overflow-y-auto">
      ...
      <div v-if="setupData && setupStep === 'BIND'" class="flex flex-col items-center gap-6 py-4">
        ...
        <div class="w-full space-y-4">
          <div class="space-y-2 flex flex-col items-center">
            ...
            <p v-if="isBinding" class="text-sm text-muted-foreground">正在验证...</p>
            <p v-if="bindErrorMessage" class="text-sm text-destructive">{{ bindErrorMessage }}</p>
          </div>
        </div>
      </div>
      <div v-else-if="setupStep === 'NAME'" class="flex flex-col gap-4 py-4">
        <div class="space-y-2">
          ...
          <Input v-model="newTotpComment" placeholder="例如：我的 iPhone" @keyup.enter="handleSaveSetupName" />
          <p class="text-xs text-muted-foreground">该名称将用于区分不同的登录设备</p>
        </div>
        <p v-if="bindErrorMessage" class="text-sm text-destructive">{{ bindErrorMessage }}</p>
        <div class="flex justify-end gap-2 mt-4">
          <Button @click="handleSaveSetupName" :disabled="isBinding">
            <span v-if="isBinding" class="mr-2 h-4 w-4 animate-spin rounded-full border-2 border-background border-t-foreground"></span>
            保存
          </Button>
        </div>
      </div>
      <div v-else class="flex items-center justify-center py-12">
        <span class="animate-spin h-5 w-5 border-2 border-primary border-t-transparent rounded-full mr-2"></span>正在生成...
      </div>
    </DialogContent>
  </Dialog>
...
async function handleSaveSetupName() {
  ...
  if (credentials.value.some(t => t.comment === newTotpComment.value && t.id !== boundTotpId.value)) {
...
async function saveComment(id: string, newText: string) {
  await runSaveComment(
    () => ConfigAPI.updateTOTPComment(id, newText),
    {
      onSuccess: () => {
