...
Function FnKnockProtectTransactionDirectory
  ...
  nsExec::ExecToStack '"$SYSDIR\icacls.exe" "$FnKnockTransactionDir" /setowner "*S-1-5-18" /T /L /Q'
  Pop $0
  Pop $1
  ${If} $0 != 0
    StrCpy $1 "unable to transfer the installer transaction directory ownership"
    Return
  ${EndIf}
  StrCpy $0 0
  StrCpy $1 ""
...
Function FnKnockWriteTransactionScript
  ...
  FileWrite $R7 "$$allowedInstallRoot = [IO.Path]::GetFullPath((Join-Path $$programFiles 'FnKnock')).TrimEnd([char]'\')$\r$\n"
  ...
  FileWrite $R7 "if (-not $$isCurrentInstallRoot -and -not $$isLegacyInstallRoot) { throw ('InstallDir must be Program Files\FnKnock or the legacy Program Files\FnKnock\current directory: ' + $$install) }$\r$\n"
  ; Enumerate each trusted parent so access-denied entries cannot be mistaken
  ; for absent paths by Directory.Exists.
  FileWrite $R7 "$$fnKnockEntry = [IO.DirectoryInfo]::new($$programFiles).EnumerateFileSystemInfos('FnKnock', [IO.SearchOption]::TopDirectoryOnly) | Select-Object -First 1$\r$\n"
  FileWrite $R7 "if ($$null -ne $$fnKnockEntry -and (($$fnKnockEntry.Attributes -band [IO.FileAttributes]::Directory) -eq 0 -or ($$fnKnockEntry.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0)) { throw 'Program Files\FnKnock is not a trusted ordinary directory' }$\r$\n"
  FileWrite $R7 "$$legacyEntry = $$null$\r$\n"
  FileWrite $R7 "if ($$isLegacyInstallRoot -and $$null -ne $$fnKnockEntry) { $$legacyEntry = [IO.DirectoryInfo]::new($$allowedInstallRoot).EnumerateFileSystemInfos('current', [IO.SearchOption]::TopDirectoryOnly) | Select-Object -First 1; if ($$null -ne $$legacyEntry -and (($$legacyEntry.Attributes -band [IO.FileAttributes]::Directory) -eq 0 -or ($$legacyEntry.Attributes -band [IO.FileAttributes]::ReparsePoint) -ne 0)) { throw 'Program Files\FnKnock\current is not a trusted ordinary directory' } }$\r$\n"
  ...
  FileWrite $R7 "$$registryEntries = @([pscustomobject]@{ Name = 'product'; SubKey = 'Software\fnknock\FnKnock'; RegKey = 'HKLM\Software\fnknock\FnKnock' }, [pscustomobject]@{ Name = 'uninstall'; SubKey = 'Software\Microsoft\Windows\CurrentVersion\Uninstall\FnKnock'; RegKey = 'HKLM\Software\Microsoft\Windows\CurrentVersion\Uninstall\FnKnock' })$\r$\n"
  ...
  FileWrite $R7 "$$shortcutEntries = @([pscustomobject]@{ Name = 'start-menu'; Path = Join-Path $$commonPrograms 'FnKnock.lnk' }, [pscustomobject]@{ Name = 'desktop'; Path = Join-Path $$commonDesktop 'FnKnock.lnk' })$\r$\n"
...
!macro NSIS_HOOK_PREINSTALL
  ...
  ${If} $INSTDIR != "$PROGRAMFILES64\FnKnock"
  ${AndIf} $INSTDIR != "$PROGRAMFILES64\FnKnock\current"
    Abort "FnKnock can only be installed under 64-bit Program Files\FnKnock. No files were changed."
  ${EndIf}
  ; Tauri enters $INSTDIR before this hook. Use a trusted working directory for
