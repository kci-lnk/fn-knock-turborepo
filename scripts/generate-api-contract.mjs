...
function validateContract(openapiPath) {
  ...
  const typedDomainOperations = new Map([
    ...
    ["post /api/admin/wol/targets/{id}/ssh/test", "WolTargetSshInputData"],
  ...
  const streamActiveIpParameters =
    document.paths?.["/api/admin/dashboard/stream-active-ips"]?.get
      ?.parameters ?? [];
  if (
    !streamActiveIpParameters.some(
      (parameter) => parameter.name === "stream" && parameter.required === true,
    )
  ) {
    throw new Error(
      "dashboard stream active IPs must require the stream query parameter",
    );
  }
  ...
  for (const [schema, fields] of [
    ["DashboardRealtimeData", ["by_host", "by_stream", "timestamp"]],
    ["DashboardHostTrafficData", ["active_ip_count"]],
    ["DashboardStreamTrafficData", ["key", "active_conns", "active_ip_count"]],
