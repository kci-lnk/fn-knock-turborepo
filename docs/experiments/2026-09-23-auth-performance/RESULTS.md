...
按冻结的 [post5-plan](post5-plan/README.md) 串行执行。规模组是原始源码对比 v5 优化源码（Rust5fdd/Go4d），参数组是相同 v5 制品的 z/reader1 对比一个参数变体；这些历史数据不包含 Go669 审计修复。均只跑一对，预热5秒、测量15秒，不能替代六对收益验收；profiling 另外采集，executor job 不等于 SQL 语句。
