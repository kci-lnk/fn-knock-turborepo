...
pub(super) async fn store_list_sessions(
    ...
    let ids = store.zrevrange_strings(SESSION_INDEX_KEY).await?;
    if ids.is_empty() {
        return Ok(Vec::new());
    }
    let keys = ids
        ...
        .collect::<Vec<_>>();
    let raws = store.mget_string_values(&keys).await?;
...
pub(super) async fn store_get_session(
    ...
    let Some(raw) = store.get_string_value(&session_data_key(id)).await? else {
        return Ok(None);
    };
    match serde_json::from_str::<TerminalSessionRecord>(&raw) {
...
pub(super) async fn store_delete_session(store: &Store, id: &str) -> anyhow::Result<()> {
    let attachment_ids = store.smembers_strings(&session_attachments_key(id)).await?;
    ...
    store.zrem_string_member(SESSION_INDEX_KEY, id).await?;
    Ok(())
}
...
pub(super) async fn store_get_attachment(
    ...
    let Some(raw) = store.get_string_value(&attachment_data_key(id)).await? else {
        return Ok(None);
    };
    match serde_json::from_str::<TerminalAttachmentRecord>(&raw) {
        ...
        Err(error) => {
            tracing::warn!(attachment_id = %id, %error, "failed to parse terminal attachment record");
            store.delete_key(&attachment_data_key(id)).await?;
            Ok(None)
        }
    }
}
...
pub(super) async fn store_refresh_attachment(
    ...
    let Some(attachment) = store_get_attachment(store, id).await? else {
        return Ok(None);
    };
    ...
    store_save_attachment(store, next, ttl_seconds)
        .await
        .map(Some)
}

pub(super) async fn store_delete_attachment(store: &Store, id: &str) -> anyhow::Result<()> {
    let Some(attachment) = store_get_attachment(store, id).await? else {
        store.delete_key(&attachment_data_key(id)).await?;
        return Ok(());
    };
