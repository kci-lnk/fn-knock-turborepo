    ...
    request_scope(state, async {
        match name {
            ...
            "auto_ip_owner_lookup" => {
                let owners =
                    auth_mobility::sql_diagnostic_list_active_sessions_by_ip(state, CLIENT_IP)
                        .await?;
                anyhow::ensure!(
...
async fn collect(state: &AppState, population: usize) -> anyhow::Result<Vec<Value>> {
    // Use the existing test-fixture replacement API: production host-mapping
    // changes require their separate CAS API, outside this measurement.
    let mut config = state.storage.store.get_config().await?;
    config["host_mappings"] = json!([{"host": HOST, "use_auth": true}]);
    config["auth_credential_settings"] =
        json!({"session_ip_mobility_enabled": true, "session_ip_mobility_window_seconds": 1200});
