...
const capabilityLabel = (value: string) => {
  ...
  return key
    ? t(`admin.cloudflareTunnel.managed.capabilities.${key}`)
    : value;
};
...
<template>
  <Card v-if="apiTokenConfigured">
    <CardHeader>
      <CardTitle>{{
        t("admin.cloudflareTunnel.managed.tunnelTitle")
      }}</CardTitle>
    </CardHeader>
    <CardContent class="space-y-5">
        ...
        <div v-if="tunnelMode === 'existing'" class="space-y-2">
          ...
          <Select v-model="selectedTunnelId">
            <SelectTrigger id="cloudflare-existing-tunnel">
              <SelectValue
                :placeholder="t('admin.cloudflareTunnel.managed.selectTunnel')"
              />
            </SelectTrigger>
            <SelectContent>
              <SelectItem
      ...
      <div
        v-if="managedState?.managed.tunnel"
        class="flex flex-wrap items-center justify-between gap-3 rounded-lg border border-destructive/30 p-4"
      >
        <div>
