import sqlite3,os,subprocess,time,pathlib,json,signal,http.client,concurrent.futures,collections,threading,shutil,sys
ROOT=pathlib.Path('/tmp/fn-knock-ab-20260918'); TOKEN='isolated-ab-token-20260918'
PATHS=['/payload','/slow','/stream','/__auth__/api/auth/bootstrap','/__auth__/api/auth/session','/__auth__/api/auth/captcha/config','/__auth__/api/auth/challenge','/__auth__/api/auth/oidc/providers','/__auth__/']
def request(path,conn=None):
 c=conn or http.client.HTTPConnection('127.0.0.1',27999,timeout=5)
 c.request('GET',path,headers={'Host':('business.local' if path in ['/payload','/slow','/stream'] else 'ab.local'),'Accept-Encoding':'identity'});r=c.getresponse();b=r.read();return r.status,r.getheader('X-Fn-Knock-Upstream-Error-Class'),len(b)
def stats(pid):
 try:
...
def run(label,gv,rv,tcache,threshold):
 ...
 config=json.loads(db.execute('select document_json from config_documents').fetchone()[0]);config['host_mappings']=[{'host':'business.local','target':'https://ab-upstream.test:28081','target_type':'proxy','use_auth':False,'suppress_toolbar':True}];config['host_mappings'] += [{'host':f'unused-{i:04d}.local','target':'https://ab-upstream.test:28081','target_type':'proxy','use_auth':False,'suppress_toolbar':True} for i in range(512)];config['run_type']=3;config['auto_manage_firewall']=False
 ...
 try:
  ...
  check=http.client.HTTPConnection('127.0.0.1',27999,timeout=5);check.request('GET','/payload',headers={'Host':'business.local'});resp=check.getresponse();observed_protocol=resp.getheader('X-AB-Protocol');resp.read();check.close()
  if observed_protocol!='HTTP/2.0':raise RuntimeError('expected HTTP/2 upstream, got '+str(observed_protocol))
  ...
  ctl=http.client.HTTPConnection('127.0.0.1',28082,timeout=5)
  def control(mode):
   ctl.request('GET','/control?mode='+mode);r=ctl.getresponse();r.read();assert r.status==200
  control('off');(ROOT/'dns-fail').touch();control('on');outage=[]
  for _ in range(5):
   try:outage.append(request('/payload'))
   except Exception as e:outage.append(str(e))
   time.sleep(.2)
  (ROOT/'dns-fail').unlink();control('on');restored=[]
  for _ in range(5):restored.append(request('/payload'));time.sleep(.2)
  summary['observed_upstream_protocol']=observed_protocol;summary['injected_outage']=outage;summary['restored']=restored;ctl.close()
  ...
  time.sleep(1)
cells=[('dns_old_old',2412,2412,7,131072),('dns_new_new',2415,2415,0,32768)]
