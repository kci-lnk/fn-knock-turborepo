...
def run(label,gv,rv):
 ...
 try:
  ...
  def load(name,seconds):
   ...
   row={'phase':name,'requests':len(times),'statuses':dict(counts),'p99_seconds':times[int(len(times)*.99)],'rust_exit':ps[1].poll(),'rust_fds':len(list(pathlib.Path(f'/proc/{ps[1].pid}/fd').iterdir())),'rust_memory':[l for l in status.splitlines() if l.startswith(('VmRSS:','VmHWM:','Threads:'))]}
   results.append(row);print(json.dumps({'label':label,**row}),flush=True)
  load('normal_external_cookie_pressure',15)
  db=sqlite3.connect(str(d/'store.sqlite3'));db.execute('BEGIN IMMEDIATE')
  try:load('sqlite_writer_locked',15)
  finally:db.rollback();db.close()
  load('after_lock_release',5)
...
for v in [2412,2415]:run('auth-pressure-unthrottled-'+str(v),v,v)
