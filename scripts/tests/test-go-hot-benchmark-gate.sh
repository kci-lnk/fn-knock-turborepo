...
assert_contains "${WORKFLOW}" '--max-latency-regression 0.10' 'latency tolerance'
assert_contains "${WORKFLOW}" '--max-bytes-regression 0.15' 'memory tolerance'
assert_contains "${WORKFLOW}" '--max-bytes-absolute-regression 1' 'memory rounding tolerance'
assert_contains "${WORKFLOW}" '-benchtime=500ms' 'benchmark sample duration'
