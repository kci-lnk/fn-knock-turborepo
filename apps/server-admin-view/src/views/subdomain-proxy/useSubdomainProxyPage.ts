...
import { useSubdomainTargetOptimization } from "./useSubdomainTargetOptimization";
...
export const useSubdomainProxyPage = () => {
  ...
  const batchActions = useSubdomainBatchActions({
    allMappings,
    isAuthServiceTarget,
    isSavingMappings,
    runSaveMappings,
  ...
  const targetOptimization = useSubdomainTargetOptimization({
    ...
    isSavingMappings: computed(
      () => isSavingMappings.value || isClearingAllSubdomainConfig.value,
    ),
    runSaveMappings,
    saveHostMappings: (mappings) => configStore.saveHostMappings(mappings),
    translate: (key, params) => (params ? t(key, params) : t(key)),
  ...
  return {
    overview: {
      ...
      openTargetOptimization: targetOptimization.openDialog,
      openToggleMappingDialog,
      refreshAllTitles,
      removeAuthService,
      resetModeForm,
