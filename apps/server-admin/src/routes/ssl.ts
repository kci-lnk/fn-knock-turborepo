...
export const sslRoutes = new Elysia({ prefix: "/api/admin/ssl" })
    ...
    .get("/ca/cert.pem", async ({ set }) => {
        ...
        set.headers['content-disposition'] = 'attachment; filename="KCI-LNK-Root-CA.pem"';
        return pem;
    })
    ...
    .get("/cert.pem", async ({ set }) => {
        ...
        set.headers['content-type'] = 'application/x-pem-file; charset=utf-8';
        set.headers['content-disposition'] = 'attachment; filename=\"server-cert.pem\"';
    ...
    .get("/cert.zip", async ({ set }) => {
        ...
        const entries = [
            { name: `server-cert.pem`, data: new TextEncoder().encode(config.ssl.cert) },
            { name: `server-key.pem`, data: new TextEncoder().encode(config.ssl.key) },
        ];
        const zipData = createZip(entries);
        return new Response(zipData, {
            headers: {
