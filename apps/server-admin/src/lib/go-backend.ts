...
export class GoBackendService {
    ...
    constructor(baseUrl: string = `http://localhost:${process.env.GO_BACKEND_PORT || 7996}` || 'http://localhost:7996') {
        this.baseUrl = baseUrl;
        this.requestTimeoutMs = this.parseTimeout(process.env.GO_BACKEND_TIMEOUT_MS, 5000);
    }
    ...
    private async request<T = unknown>(
        ...
        try {
            ...
            if (!res.ok) {
                ...
                const suppressed = options?.suppressStatusLog?.includes(res.status) ?? false;
                if (!suppressed) {
                    console.error(`[GoBackend] ${method} ${path} failed: ${res.status} ${res.statusText}`, text);
                }
                return { success: false, code: res.status, message: `${res.status} ${res.statusText}` };
            }

            try {
                return (await res.json()) as GoResponse<T>;
            } catch (e: any) {
                console.error(`[GoBackend] ${method} ${path} invalid JSON response:`, e);
                return { success: false, code: 502, message: "Invalid JSON from go-backend" };
            }
        } catch (e: any) {
            if (e?.name === "AbortError") {
                console.error(`[GoBackend] ${method} ${path} timeout after ${timeoutMs}ms`);
                return { success: false, code: 504, message: `Go backend timeout (${timeoutMs}ms)` };
            }
    ...
    async setDefaultRoute(route: string): Promise<GoResponse> {
        return this.request('/api/config/default-route', 'POST', { default_route: route });
    }

    async getProxyProtocolForce(): Promise<GoResponse<ProxyProtocolForceResponse>> {
        return this.request<ProxyProtocolForceResponse>('/api/config/proxy-protocol');
    }

    async setProxyProtocolForce(proxy_protocol_force: boolean): Promise<GoResponse<ProxyProtocolForceResponse>> {
        return this.request<ProxyProtocolForceResponse>('/api/config/proxy-protocol', 'POST', { proxy_protocol_force } satisfies ProxyProtocolForceRequest);
    }
    ...
    async setSSL(cert: string, key: string): Promise<GoResponse> {
        return this.request('/api/ssl', 'POST', { cert, key } satisfies SSLRequest);
    }
    ...
    async allowIP(ip: string): Promise<GoResponse> {
        return this.request('/api/iptables/allow', 'POST', { ip } satisfies IpRequest);
    }

    async removeIP(ip: string): Promise<GoResponse> {
        return this.request('/api/iptables/remove', 'POST', { ip } satisfies IpRequest);
    }

    async blockIP(ip: string): Promise<GoResponse> {
        return this.request('/api/iptables/block', 'POST', { ip } satisfies IpRequest);
    }
