      ...
      >
        ...
        <div class="scan-pressure-track">
          <div class="scan-pressure-visual" aria-hidden="true">
            ...
            <canvas ref="matrixCanvas" class="scan-pressure-canvas"></canvas>
          </div>
...
const graphicsFallback = ref(false);
...
function restoreAutomaticMode() {
  if (automatic.value || loading.value || saving.value || props.disabled)
    return;
  clearPendingSave();
...
function initializePortMatrix() {
  ...
  if (!context) {
    graphicsFallback.value = true;
    return;
  }
  graphics = context;
  graphicsFallback.value = false;
  reducedMotion = window.matchMedia("(prefers-reduced-motion: reduce)").matches;
  canvas.addEventListener("webglcontextlost", handleMatrixContextLost);
  canvas.addEventListener("webglcontextrestored", handleMatrixContextRestored);
  try {
  ...
  visualTier = displayedIndex.value;
...
function renderPortMatrix(timestamp: number) {
  ...
  const width = matrixCanvas.value.width;
  const height = matrixCanvas.value.height;
...
function requestMatrixFrames() {
  if (!graphics || loopActive || !props.open || visualTier !== 3) return;
  loopActive = true;
  animationToken = requestAnimationFrame(renderPortMatrix);
}
