...
def run(label,gv,rv,tcache,threshold):
 ...
 try:
  ...
  time.sleep(1)
cells=[('old_old',2412,2412,7,131072),('new_new_legacyalloc',2415,2415,7,131072),('old_new',2412,2415,7,131072),('new_old',2415,2412,7,131072),('new_new_tcache0',2415,2415,0,131072),('new_new_fpkalloc',2415,2415,0,32768),('new_new_fpkalloc_repeat',2415,2415,0,32768),('old_old_repeat',2412,2412,7,131072)]
for cell in cells:
 if len(sys.argv)==1 or cell[0] in sys.argv[1:]:run(*cell)
