export const zhCNServer = {
  ...
  admin: {
    ...
    dockerPanel: {
      passwordNotNeeded: "当前部署不需要设置管理面板密码",
      setPasswordFailed: "设置管理面板密码失败",
      passwordChangeUnsupported: "当前部署不支持修改管理面板密码",
    ...
    firewallAdditionalPorts: {
      loadFailed: "加载额外放行端口配置失败",
      saveFailed: "保存额外放行端口配置失败",
      updateFailedRolledBack:
        "应用额外放行端口失败，已恢复之前的配置和防火墙：{message}",
    ...
    runtimeConfigRoutes: {
      ...
      loadWolFeatureFailed: "加载远程唤醒功能配置失败",
      saveWolFeatureFailed: "保存远程唤醒功能配置失败",
      syncWolFeatureFailed: "同步远程唤醒功能到网关失败",
      invalidWolFeature: "远程唤醒功能配置无效",
    ...
    captcha: {
      ...
      powDifficultyInvalid:
        "PoW 难度必须是 10000 到 1000000 之间、步长为 10000 的整数",
      powEnabledBooleanRequired: "不常用地难度开关必须是布尔值",
      powUncommonDifficultyTooLow: "不常用地难度不能低于基础难度",
    },
    ipLocation: {
    ...
    hostMappings: {
      ...
      renamePreviousHostInvalid: "Host 映射 {host} 的原域名无效",
      renameDestinationExists:
        "Host 映射 {host} 已存在，不能从 {previousHost} 重命名",
      renamePreviousHostStillPresent:
        "原 Host 映射 {previousHost} 仍在列表中，无法作为重命名来源",
      renamePreviousHostMissing: "原 Host 映射 {previousHost} 不存在",
      renamePreviousHostClaimed:
        "原 Host 映射 {previousHost} 被多条映射重复认领",
      targetInvalid:
    ...
    totpImport: {
      ...
      accountsArray: "账号凭证列表必须是数组",
      passwordArray: "账号密码凭证列表必须是数组",
      countExceeded: "单次最多导入 {max} 个 TOTP 凭证",
      accountCountExceeded: "单次最多导入 {max} 个账号凭证",
      passwordCountExceeded: "单次最多导入 {max} 个账号密码凭证",
    },
    passkeys: {
  ...
  ldap: {
    catalog: {
      ...
      customLabel: "自定义 LDAP",
    },
    listProvidersFailed: "获取 LDAP 提供商失败",
    providerPayloadObject: "提供商配置必须是对象",
    createProviderFailed: "创建 LDAP 提供商失败",
    providerNotFound: "LDAP 提供商不存在",
    loadProviderFailed: "加载 LDAP 提供商失败",
    updateProviderFailed: "更新 LDAP 提供商失败",
    deleteProviderFailed: "删除 LDAP 提供商失败",
    connectionTestSuccess: "LDAP 连接测试成功",
    testProviderFailed: "LDAP 连接测试失败",
    testCredentialsRequired: "目录用户名和密码必须同时提供",
    listBindingsFailed: "获取 LDAP 绑定失败",
    bindingNotFound: "LDAP 绑定不存在",
    deleteBindingFailed: "删除 LDAP 绑定失败",
    invitationFieldsRequired: "必须选择 TOTP 凭据和 LDAP 提供商",
    loadTotpFailed: "加载 TOTP 凭据失败",
    totpMissing: "TOTP 凭据不存在",
    providerUnavailable: "LDAP 提供商不可用",
    loadConfigFailed: "加载配置失败",
    inviteUrlBuildFailed: "生成 LDAP 邀请链接失败",
    createInviteFailed: "创建 LDAP 邀请失败",
    loginMethodUnavailable: "当前登录模式不支持 LDAP 登录",
    inviteInvalid: "LDAP 邀请无效",
    inviteExpired: "LDAP 邀请已过期或已使用",
    serviceUnavailable: "目录服务暂时不可用",
    invalidCredentialsWithRetry: "目录凭据无效，请在 {seconds} 秒后重试。",
    invalidCredentials: "目录凭据无效或账号尚未绑定",
    bindingConflict: "该目录身份已绑定，或邀请已被使用",
    bindingFailed: "绑定目录身份失败",
    createSessionFailed: "创建登录会话失败",
    loginSuccessful: "LDAP 登录成功",
  },
  subdomainMode: {
  ...
  maintenanceBackup: {
    ...
    syncSteps: {
      ...
      smartConnect: "智能连接",
      fnosPortIconHijack: "飞牛端口图标接管",
      fnosNetworkTuning: "飞牛网络调优",
      transactionFinalize: "备份导入事务收尾",
      locale: "语言配置",
  ...
  ddns: {
    ...
    domainTargets: {
      invalidDomain: "完整域名格式无效: {domain}",
      tooMany: "完整域名最多只能填写两个",
      invalidPair: "填写两个完整域名时，必须是通配符与对应基础域名组合",
      mismatchedPair: "通配符域名与基础域名不匹配",
      pairUnsupported: "{provider} 不支持同时更新通配符与基础域名",
      rootMissing: "使用双域组合前必须填写 {field}",
      rootMismatch:
        "双域组合的基础域名不在 {field} 管理范围内（Zone {expected}，组合 {actual}）",
      allSucceeded: "共 {count} 个域名",
      itemSucceeded: "{domain}: 成功",
      itemFailed: "{domain}: 失败（{detail}）",
    },
    ...
    providers: {
      ...
      cloudflare: {
        ...
        zoneLookupFailed: "Cloudflare Zone 查询失败: {detail}",
        zoneMismatch:
          "双域组合的基础域名不在 Cloudflare Zone 内（Zone {expected}，组合 {actual}）",
      ...
      esa: {
        ...
        siteLookupFailed: "阿里云 ESA 站点查询失败: {detail}",
        siteMismatch:
          "配置的 Site ID 与站点查询结果不一致（配置 {expected}，查询到 {actual}）",
      ...
      dynu: {
        fields: {
          ...
          domain: {
            description:
              "要更新的完整 Dynu hostname；双域组合的基础域名必须已在 Dynu 中配置为独立 DDNS Service，不能是该 Service 下的普通子域。更新时会设置其 IP 并启用 Wildcard Alias，不会额外创建基础域名记录",
          },
          group: {
      ...
      edgeone: {
        ...
        zoneLookupFailed: "EdgeOne 站点查询失败: {detail}",
        zoneMismatch:
          "双域组合的基础域名不在 EdgeOne Zone 内（Zone {expected}，组合 {actual}）",
  ...
  notifications: {
    ...
    templates: {
      ...
      terminalActions: {
        targetCreated: "已创建 SSH 目标",
        targetUpdated: "已更新 SSH 目标",
        targetDeleted: "已删除 SSH 目标",
        hostKeyConfirmed: "已确认主机指纹",
        connectionTestSucceeded: "SSH 连接测试成功",
        connectionTestFailed: "SSH 连接测试失败",
        localTerminalEnabled: "已启用本机终端",
        localTerminalDisabled: "已禁用本机终端",
        sessionCreationStarted: "已开始创建终端会话",
        sessionCreationFailed: "终端会话创建失败",
        sessionEnded: "终端会话已结束",
        sessionExited: "Shell 已退出",
        sessionLost: "终端会话连接丢失",
      },
      credential: "凭证",
    ...
    providers: {
      catalog: {
        ...
        webhook: {
          label: "Webhook 通知",
          description: "向任意 HTTP 地址发送标准或自定义 JSON、文本通知。",
          fields: {
            url: {
              label: "Webhook 地址",
              description: "接收 Webhook 请求的目标地址。",
            },
            method: {
              label: "请求方法",
            },
            ...
            body_config: {
              label: "请求体",
              description:
                "使用标准通知 Body，或通过变量组装自定义 JSON、文本 Body。",
            },
            endpoint_path: {
              label: "附加路径",
            ...
            body_override: {
              label: "请求体覆盖",
              description: "继承提供商 Body，或为当前通知目标单独覆盖。",
            },
            extra_headers_json: {
              label: "额外请求头 JSON",
          ...
          errors: {
            ...
            invalidBodyConfig: "Webhook Body 配置格式不正确",
            invalidBodyMode: "Webhook Body 模式 {mode} 不正确",
            invalidBodyFormat: "Webhook Body 格式 {format} 不正确",
            bodyTemplateRequired: "自定义 Webhook Body 模板不能为空",
            bodyTemplateTooLarge: "Webhook Body 模板不能超过 {max} 字节",
            invalidBodyTemplateJson: "Webhook JSON Body 模板不是合法 JSON",
            unclosedBodyVariable: "Webhook Body 中存在未闭合变量",
            invalidBodyVariable: "Webhook Body 变量 {path} 格式不正确",
            tooManyBodyVariables: "Webhook Body 模板最多允许 {max} 个变量",
            invalidBodyContentType: "Webhook Body 的 Content-Type 不合法",
            bodyContentTypeTooLong:
              "Webhook Body 的 Content-Type 不能超过 {max} 字节",
            duplicateRenderedBodyKey: "Webhook Body 变量产生了重复的 JSON 键",
            renderedBodyTooLarge: "渲染后的 Webhook Body 不能超过 {max} 字节",
            invalidBodySample: "Webhook Body 样例上下文必须是 JSON 对象",
            bodySampleTooLarge: "Webhook Body 样例上下文不能超过 {max} 字节",
          },
        },
        magicpush: {
          label: "MagicPush魔法推送",
